...
class DenizbankParser(BaseParser):
    ...
    def parse(self):
        raw = self.text
        # Metni temizleyerek tek satır haline getiriyoruz (Regex daha iyi yakalar)
        ...
        # 4. Gönderen İsmi (Garantili Regex)
        # 'Adı Soyadı' ile 'İşlem Türü' arasındaki her şeyi alır
        m_g = re.search(r"Adı\s*Soyadı\s+(.*?)(?=\s*İşlem\s*Türü)", clean_raw, re.I | re.S)
        if m_g:
            self.data["gonderen"] = " ".join(m_g.group(1).split()).strip()

        # 5. Alıcı İsmi
        m_a = re.search(r"Alıcı\s*Adı\s*Soyadı\s+(.*?)(?=\s*Tutar)", clean_raw, re.I | re.S)
        if m_a:
        ...
        # 7. Gönderen IBAN (Metindeki ilk genel IBAN)
